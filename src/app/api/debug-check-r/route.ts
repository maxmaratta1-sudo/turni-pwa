...
export async function DELETE(req: NextRequest) {
  ...
  const { data: stef } = await supabaseAdmin.from('employees').select('id').ilike('nome', 'Stefania').single()
  const scheduleId = '5c4daad2-427b-4f26-8847-dcc3a3bc3896'
  if (!stef) return NextResponse.json({ error: 'not found' })

  await supabaseAdmin.from('unavailabilities').delete().eq('employee_id', stef.id).eq('data', '2026-08-03')
  await supabaseAdmin.from('shifts').delete().eq('employee_id', stef.id).eq('schedule_id', scheduleId).eq('data', '2026-08-09')
